...
def get_model_baseline(
    ...
    if('first_glance' in features_list):
        ...
        edges_first_glance = tf.keras.layers.Dense(size_hidden_state, input_shape=(
            size_hidden_state,), use_bias=True, name='edges_first_glance')
    ...
    if('bodies_age' in features_list):
        ...
        edges_bodies_age = tf.keras.layers.Dense(size_hidden_state, input_shape=(
            size_hidden_state,), use_bias=True, name='edges_bodies_age')
    ...
    if('bodies_clothing' in features_list):
        ...
        edges_bodies_clothing = tf.keras.layers.Dense(size_hidden_state, input_shape=(
            size_hidden_state,), use_bias=True, name='edges_bodies_clothing')
    ...
    if('bodies_gender' in features_list):
        ...
        edges_bodies_gender = tf.keras.layers.Dense(size_hidden_state, input_shape=(
            size_hidden_state,), use_bias=True, name='edges_bodies_gender')
    ...
    if('bodies_activity' in features_list):
        ...
        edges_bodies_activity = tf.keras.layers.Dense(size_hidden_state, input_shape=(
            size_hidden_state,), use_bias=True, name='edges_bodies_activity')
    ...
    if('context_activity' in features_list):
        ...
        edges_context_activity = tf.keras.layers.Dense(size_hidden_state, input_shape=(
            size_hidden_state,), use_bias=True, name='edges_context_activity')
    ...
    if('context_emotion' in features_list):
        ...
        edges_context_emotion = tf.keras.layers.Dense(size_hidden_state, input_shape=(
            size_hidden_state,), use_bias=True, name='edges_context_emotion')
    ...
    if('objects_attention' in features_list):
        ...
        edges_objects_attention = tf.keras.layers.Dense(size_hidden_state, input_shape=(
            size_hidden_state,), use_bias=True, name='edges_objects_attention')
    ...
    for step in range(num_steps):
        ...
        if('first_glance' in features_list):
            head = tail
            tail = tail + 1
            message_first_glance = edges_dropout(
                edges_first_glance(hidden_state[head:tail]))
            messages.append(message_first_glance)

        if('bodies_age' in features_list):
            head = tail
            tail = tail + 2
            message_bodies_age = edges_dropout(
                edges_bodies_age(hidden_state[head:tail]))
            messages.append(message_bodies_age)

        if('bodies_clothing' in features_list):
            head = tail
            tail = tail + 2
            message_bodies_clothing = edges_dropout(
                edges_bodies_clothing(hidden_state[head:tail]))
            messages.append(message_bodies_clothing)

        if('bodies_gender' in features_list):
            head = tail
            tail = tail + 2
            message_bodies_gender = edges_dropout(
                edges_bodies_gender(hidden_state[head:tail]))
            messages.append(message_bodies_gender)

        if('bodies_activity' in features_list):
            head = tail
            tail = tail + 2
            message_bodies_activity = edges_dropout(
                edges_bodies_activity(hidden_state[head:tail]))
            messages.append(message_bodies_activity)

        if('context_activity' in features_list):
            head = tail
            tail = tail + 1
            message_context_activity = edges_dropout(
                edges_context_activity(hidden_state[head:tail]))
            messages.append(message_context_activity)

        if('context_emotion' in features_list):
            head = tail
            tail = tail + 1
            message_context_emotion = edges_dropout(
                edges_context_emotion(hidden_state[head:tail]))
            messages.append(message_context_emotion)

        if('objects_attention' in features_list):
            head = tail
            message_objects_attention = edges_dropout(
                edges_objects_attention(hidden_state[head:]))
